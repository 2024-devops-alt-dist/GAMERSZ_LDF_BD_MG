...
const ChatroomPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const chatroomData: ChatroomApiResponse = await chatroomResponse.json();
    ...
    const fetchMessages = async (chatroomId: string) => {
      try {
        ...
        const messagesData: MessagesApiResponse = await messagesResponse.json();
  ...
  const isCurrentUser = (message: Message): boolean => {
    ...
    return (
      user.email === message.senderId || user.username === message.senderId
    );
  };
